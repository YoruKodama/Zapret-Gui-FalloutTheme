# 🎮 Zapret GUI для Discord и YouTube

**Графический интерфейс для обхода блокировок Discord и YouTube**
...
Этот скрипт:
- ✅ Автоматически найдет .exe или Python версию
- ✅ Установит все зависимости если нужно
- ✅ Запустит GUI
- ✅ Не требует никаких действий от вас!

### 📦 Portable версия (БЕЗ Python!)

**Если у вас есть готовый .exe файл:**

1. Скачайте готовый `ZapretMatrixGUI.exe`
2. Запустите `START_GUI_PORTABLE.bat` или двойной клик на `ZapretMatrixGUI.exe`
3. Готово! ✨

**Преимущества:**
- ✅ **Не нужен Python** - все включено в один .exe файл
- ✅ **Двойной клик** - просто запустите
- ✅ **Portable** - можно запускать с флешки
- ✅ **Размер:** ~50-80 MB (все библиотеки внутри)

### 🐍 Python версия (автоматическая установка)

Если у вас есть Python:

```batch
# Просто запустите - зависимости установятся сами:
START_GUI.bat
```
...
Хотите собрать .exe самостоятельно?

```batch
# Просто запустите:
build_exe.bat

# Создать дистрибутив для распространения:
...
## ❓ Быстрая помощь по GUI

### Не запускается?

1. **Запустите от имени администратора** (ПКМ → "Запуск от имени администратора")
2. **Добавьте в исключения антивируса** (папку с программой)
3. **Проверьте файлы:** `bin/winws.exe` и `bin/WinDivert64.sys` должны быть на месте

### Не работает обход?

1. **Попробуйте другую стратегию** в выпадающем списке
2. **Перезапустите:** Деактивация → Активировать
3. **Перезагрузите компьютер**

Полная инструкция: [USER_GUIDE_RU.txt](USER_GUIDE_RU.txt)

---

## 📚 Документация GUI

- **[MATRIX_GUI_README.md](MATRIX_GUI_README.md)** - подробное описание интерфейса
- **[BUILD_INSTRUCTIONS.md](BUILD_INSTRUCTIONS.md)** - инструкция по сборке .exe
- **[USER_GUIDE_RU.txt](USER_GUIDE_RU.txt)** - краткое руководство пользователя
...
- Windows 10/11
- Python 3.8+
- Зависимости из requirements_gui.txt
...
- **Packaging**: PyInstaller 6.0+ (для .exe)
